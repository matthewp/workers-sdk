...
describe("WorkerOptionsSchema", () => {
	...
	describe("dev config", () => {
			...
			expect(
				WorkerOptionsSchema.safeParse({
					...
					dev: {
						...
						outboundService: "outbound-worker",
						unsafeDirectSockets: [
		...
		test("accepts outboundService as a function", ({ expect }) => {
			expect(
				WorkerOptionsSchema.safeParse({
					config: workerConfigBase(),
	...
	test("accepts shared options", ({ expect }) => {
		expect(
			SharedOptionsSchema.safeParse({
				...
				persistRoot: ".wrangler/state/v3",
				containerEngine: {
					localDocker: { socketPath: "/var/run/docker.sock" },
				},
				log: { loggerLevel: 0, log: () => {} },
				handleStructuredLogs: () => {},
				moduleFallbackService: () => new Response("fallback"),
				devRegistryPath: "/tmp/dev-registry",
				proxySharedSecret: "secret",
				triggerHandlers: true,
				enableLocalExplorer: true,
