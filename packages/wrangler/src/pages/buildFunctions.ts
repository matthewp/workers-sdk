 ...
 * @param fallbackTelemetry - Fallback telemetry label for unrecognised errors
 */
function rethrowAsWranglerError(e: unknown, fallbackTelemetry: string): never {
	...
	throw new FunctionsBuildError(e instanceof Error ? e.message : String(e), {
		telemetryMessage: fallbackTelemetry,
	});
}
...
> & {
	...
	try {
		...
		rethrowAsWranglerError(e, "pages functions route config failed");
	}
	...
	try {
		...
		rethrowAsWranglerError(e, "pages functions write routes module failed");
	}
